// api/wick-memory.js — Wick's memory endpoint, two operations on one file to
// stay under Vercel's Hobby-plan 12-Serverless-Function cap: GET lists what
// he's filed away (read-only; wick_memory has RLS with no policies, so the
// anon key used for campaigns/links/docket_tasks gets zero access on
// purpose — this goes through the service-role key only, same as writes).
// POST closes a session, distilling it into at most four wick_memory rows
...
export default async function handler(req, res) {
  if (req.method === 'GET') return listMemory(req, res);
  if (req.method === 'POST') return closeSession(req, res);
