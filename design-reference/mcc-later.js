...
function screenRevenue(s) {
  const rows = inBrand(REVENUE, s.brand).sort((a,b) => b.revenue - a.revenue);
  ...
  return `<div class="stack">
    <div class="card pad"><div class="between"><div><h2 class="h2">Cost against closed revenue</h2>
      <p class="sub" style="margin-top:5px;max-width:520px">Lead counts only tell half the story. This view waits on Pipedrive deal values, shown here with sample figures so the shape is settled.</p></div>
      <span class="pill mute">Sample data</span></div></div>
    <div class="kpis">
    ...
    <div class="card scroll-x"><table><thead><tr><th>Channel</th><th>Brand</th><th>Spend</th><th>Leads</th><th>Cost / lead</th><th>Closed</th><th>Revenue</th><th>Return</th><th style="width:24%">Share</th></tr></thead>
      <tbody>${rows.map(r => `<tr><td style="font-weight:500">${esc(r.channel)}</td><td>${bdot(r.brand)}</td>
        ...
        <td class="mono" style="color:${r.cost && r.revenue / r.cost > 5 ? '#7cc3b2' : 'var(--text-2)'}">${r.cost ? (r.revenue / r.cost).toFixed(1) + '×' : '∞'}</td>
...
function screenLoading() {
  ...
  return `<div class="stack" aria-busy="true">
    <div class="goals two">${[0,1].map(() => `<div class="card pad">${sk('40%', 12)}${sk('58%', 44, 18)}${sk('100%', 6, 20)}${sk('30%', 10, 12)}</div>`).join('')}</div>
    <div class="card"><div class="pad" style="border-bottom:1px solid var(--line)">${sk('34%', 13)}</div>
      <div class="pad" style="display:flex;flex-direction:column;gap:18px">${[92,76,60,44].map(w => `<div>${sk(w * .5 + '%', 11)}${sk('100%', 5, 8)}</div>`).join('')}</div></div>
    <div class="card"><div class="pad" style="border-bottom:1px solid var(--line)">${sk('28%', 13)}</div>
      <div class="pad" style="display:flex;flex-direction:column;gap:20px">${[0,1,2].map(() => `<div>${sk('62%', 12)}${sk('40%', 10, 9)}</div>`).join('')}</div></div>
  </div>`;
}
